...
def average_of_sub_lists(lst):
    ...
    Returns:
    - list: A list of averages for average_of_sub_lists with more than 3 elements.
    """
    averages = []
    for sublist in lst:
        if len(sublist) >= 3:  # Only consider average_of_sub_lists with more than 3 elements
...
def get_important_levels(df_list, tolerance):
    ...
    important_levels = average_of_sub_lists(grouped_levels)
...
def read_csv_with_default(file_path,delimiter):
    """
    Reads a CSV file with default encoding 'utf-16' and delimiter ';',
    detects and prints the encoding used, and prints the resulting DataFrame.
